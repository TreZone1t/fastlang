...
impl CraneliftBackend {
    ...
    fn compile_function(&mut self, ir_func: &IRFunction) {
        ...
        for (_name, ty) in &ir_func.params {
            self.ctx.func.signature.params.push(cranelift::prelude::AbiParam::new(Self::map_type(ty)));
        }
        if ir_func.return_type != IRType::Void {
            self.ctx.func.signature.returns.push(cranelift::prelude::AbiParam::new(Self::map_type(&ir_func.return_type)));
        }

        let builder = cranelift_frontend::FunctionBuilder::new(&mut self.ctx.func, &mut self.builder_context);
