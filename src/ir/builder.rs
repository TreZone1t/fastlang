...
use crate::parser::ast::{Stmt, Expr, TypeMetadata, TypeNode};
...
impl<'a> IRBuilder<'a> {
        ...
        for stmt in stmts {
            match stmt {
                Stmt::FnDecl { name, params, return_type, body, .. } => {
                    ...
                    for param in params {
                        let ty = param.type_node.as_ref().map(|tn| IRType::from_ast(tn)).unwrap_or(IRType::Int32);
    ...
    fn visit_stmt(&mut self, stmt: &Stmt) {
        match stmt {
            Stmt::VarDecl { name, type_node, value, .. } => {
                let ty = type_node.as_ref().map(|tn| IRType::from_ast(tn)).unwrap_or(IRType::Int32);
