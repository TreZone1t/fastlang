use crate::parser::ast::{TypeNode, BaseType};
...
impl IRType {
    pub fn from_ast(tn: &TypeNode) -> Self {
        match tn {
            TypeNode::Simple(type_ref) => match &type_ref.base_type {
                BaseType::Int => {
                    if type_ref.size == Some(64) { IRType::Int64 } else { IRType::Int32 }
                },
                BaseType::Float => {
                    if type_ref.size == Some(64) { IRType::Float64 } else { IRType::Float32 }
                },
                BaseType::Bool => IRType::Bool,
                BaseType::Char => IRType::Int32,
                BaseType::Void => IRType::Void,
                BaseType::Array(inner) => IRType::Array(Box::new(IRType::Pointer(Box::new(IRType::Void)))), // TODO: map inner properly
                BaseType::Object(name) => IRType::Object(name.clone()),
                BaseType::Custom(name) => IRType::CustomScope(name.clone()),
                BaseType::Name(_) | BaseType::Type(_) => IRType::Pointer(Box::new(IRType::Void)),
                _ => IRType::Pointer(Box::new(IRType::Void)),
            },
            TypeNode::Generic(gen) => IRType::Generic(format!("{:?}", gen.base_type)),
        }
    }
}
