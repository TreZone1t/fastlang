...
impl Parser {
    ...
    pub(crate) fn parse_disable(
        ...
        let mut temp_list: Vec<Setting> = Vec::new();
        ...
        while !self.is_at_end() {
            ...
            if self.is_valid_setting(current_kind.clone()) {
                let current_setting = Setting::from_token(current_kind.clone());
                temp_list.push(current_setting);
            }
            self.advance();
            let next_kind = self.peek().kind.clone();
            if next_kind == TokenKind::Comma {
        ...
        // we need now to filter the settings and enable the handles
        for e in temp_list.clone() {
            match e {
                Setting::CustomDisplay => handles.retain(|h| h != &HandleMethods::Display),
                Setting::Call => handles.retain(|h| h != &HandleMethods::Call),
                Setting::CustomIndexAccess => handles.retain(|h| h != &HandleMethods::IndexAccess),
                Setting::CustomIterator => {
                    handles.retain(|h| h != &HandleMethods::Iterator);
                    handles.retain(|h| h != &HandleMethods::Next);
                }
                Setting::CustomOperators => {
                    handles.retain(|h| h != &HandleMethods::Add);
                    handles.retain(|h| h != &HandleMethods::Sub);
                    handles.retain(|h| h != &HandleMethods::Mul);
                    handles.retain(|h| h != &HandleMethods::Div);
                    handles.retain(|h| h != &HandleMethods::Mod);
                }
                Setting::Break => {
                    if !flags.contains(&Flag::HasBreak) {
                        flags.retain(|f| f != &Flag::HasBreak);
                    }
                    handles.retain(|h| h != &HandleMethods::Break)
                }
                Setting::Error => {
                    if !flags.contains(&Flag::HasError) {
                        flags.retain(|f| f != &Flag::HasError);
                    }
                    handles.retain(|h| h != &HandleMethods::Error)
                }
                Setting::Throw => {
                    if !flags.contains(&Flag::HasThrow) {
                        flags.retain(|f| f != &Flag::HasThrow);
                    }
                    handles.retain(|h| h != &HandleMethods::Error)
                }
                Setting::Leave => handles.retain(|h| h != &HandleMethods::Leave),
                Setting::Yield => handles.retain(|h| h != &HandleMethods::Yield),
                Setting::Data => handles.retain(|h| h != &HandleMethods::Data),
                Setting::Length => handles.retain(|h| h != &HandleMethods::Length),
                Setting::Size => handles.retain(|h| h != &HandleMethods::Size),
                Setting::Exit => {
                    if !flags.contains(&Flag::HasExit) {
                        flags.retain(|f| f != &Flag::HasExit);
                    }
                    handles.retain(|h| h != &HandleMethods::Exit)
                }
                _ => {}
            }
        }
        if temp_list.contains(&Setting::Custom) {
            setting.retain(|s| s != &Setting::CustomIndexAccess);
            setting.retain(|s| s != &Setting::CustomConstructor);
            setting.retain(|s| s != &Setting::CustomGeneric);
            setting.retain(|s| s != &Setting::CustomIterator);
            setting.retain(|s| s != &Setting::CustomDisplay);
            setting.retain(|s| s != &Setting::CustomOperators);
            // we need to remove the custom from the list
            setting.retain(|s| s != &Setting::Custom);
        } else if temp_list.contains(&Setting::All) {
            self.disable_all(setting)?;
            setting.retain(|s| s != &Setting::All);
        } else if temp_list.contains(&Setting::OOP) {
            self.disable_oop(setting)?;
            setting.retain(|s| s != &Setting::OOP);
        } else if setting.contains(&Setting::Function) {
            self.disable_function(setting)?;
            setting.retain(|s| s != &Setting::Function);
        }
        self.consume(TokenKind::RBracket, "Expected ']' after enable list")?;
        self.consume(TokenKind::SemiColon, "Expected ';' after enable statement")?;
    ...
    pub(crate) fn disable_all(&mut self, setting: &mut Vec<Setting>) -> Result<(), String> {
        setting.retain(|s| !matches!(s, Setting::Constructor));
        setting.retain(|s| !matches!(s, Setting::Private));
        setting.retain(|s| !matches!(s, Setting::Public));
        setting.retain(|s| !matches!(s, Setting::Static));
        setting.retain(|s| !matches!(s, Setting::Extends));
        setting.retain(|s| !matches!(s, Setting::Param));
        setting.retain(|s| !matches!(s, Setting::Statement));
        setting.retain(|s| !matches!(s, Setting::Return));
        setting.retain(|s| !matches!(s, Setting::Break));
        setting.retain(|s| !matches!(s, Setting::Case));
        setting.retain(|s| !matches!(s, Setting::CustomIndexAccess));
        setting.retain(|s| !matches!(s, Setting::CustomConstructor));
        setting.retain(|s| !matches!(s, Setting::CustomGeneric));
        setting.retain(|s| !matches!(s, Setting::CustomIterator));
        setting.retain(|s| !matches!(s, Setting::CustomDisplay));
        setting.retain(|s| !matches!(s, Setting::CustomOperators));
        setting.retain(|s| !matches!(s, Setting::Error));
        setting.retain(|s| !matches!(s, Setting::Handle));
        setting.retain(|s| !matches!(s, Setting::Variants));
        setting.retain(|s| !matches!(s, Setting::Length));
        setting.retain(|s| !matches!(s, Setting::Data));
        Ok(())
    }
    ...
    pub(crate) fn disable_oop(&mut self, setting: &mut Vec<Setting>) -> Result<(), String> {
        setting.retain(|s| !matches!(s, Setting::Constructor));
        setting.retain(|s| !matches!(s, Setting::Private));
        setting.retain(|s| !matches!(s, Setting::Public));
        setting.retain(|s| !matches!(s, Setting::Static));
    ...
    pub(crate) fn disable_function(&mut self, setting: &mut Vec<Setting>) -> Result<(), String> {
        setting.retain(|s| !matches!(s, Setting::Param));
        setting.retain(|s| !matches!(s, Setting::Statement));
        setting.retain(|s| !matches!(s, Setting::Return));
        Ok(())
    ...
    pub(crate) fn parse_field_block(
        ...
        let mut generics = generics.unwrap_or(Vec::new());
        while !self.is_at_end() && self.peek().kind != TokenKind::RBrace {
            ...
            if token == TokenKind::Fn {
                ...
                if matches!(self.peek().kind, TokenKind::Identifier(_)) {
                    ...
                    if generics.contains(&TypeNode::Simple(TypeRef {
                        base_type: BaseType::from_str(&type_name),
                        size: None,
                    })) {
                        return_type = TypeNode::Simple(TypeRef {
                            base_type: BaseType::from_str(&type_name),
                            size: None,
                        });
                    } else {
            ...
            if matches!(token, TokenKind::Identifier(_)) {
                ...
                if generics.contains(&TypeNode::Simple(TypeRef {
                    ...
                    let type_ = TypeNode::Simple(TypeRef {
                        base_type: BaseType::from_str(&type_name),
                        size: None,
                    });
            ...
            println!("DEBUG: token in parse_field_block: {:?} at line {}, column {}", token, self.peek().line, self.peek().column);
            return Err(format!("Syntax Error: Expected fn or var declaration inside public block, found {:?}", token));
        }
    ...
    pub(crate) fn parse_generics(
        &mut self,
        generics: &mut Option<Vec<TypeNode>>,
    ) -> Result<(), String> {
        if generics.is_none() {
            *generics = Some(Vec::new());
        }
        let generics = generics.as_mut().unwrap();
        ...
        while !self.is_at_end() && self.peek().kind != TokenKind::RBrace {
            ...
            if matches!(token, TokenKind::Identifier(_)) {
                ...
                generics.push(TypeNode::Simple(TypeRef {
                    base_type: BaseType::from_str(&type_name),
                    size: None,
                }));
                continue;
