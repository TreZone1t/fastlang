...
impl Parser {
    pub(crate) fn parse_public_block(&mut self) -> Result<Vec<Stmt>, String> {
        ...
        while !self.is_at_end() && self.peek().kind != TokenKind::RBrace {
            if let Some(stmt) = self.parse_statement()? { block.push(stmt); }
        }
    ...
    pub(crate) fn parse_handle_block(
        ...
        while !self.is_at_end() && self.peek().kind != TokenKind::RBrace {
            ...
            if stmt_opt.is_none() { continue; }
            let stmt = stmt_opt.unwrap();
            if let Stmt::FnDecl { name: ref fn_name, .. } = stmt {
                ...
                if found {
                    ...
                    return Err(format!("Syntax Error: '{}' handle not allowed in this scope.", fn_name));
                }
            } else {
    ...
    pub(crate) fn parse_constructor_decl_block(&mut self) -> Result<Option<crate::parser::ast::ConstructorDecl>, String> {
            ...
            if self.peek().kind == TokenKind::TypeParam {
                ...
                self.consume(TokenKind::LBrace, "Expected '{' to open constructor param block")?;
                while !self.is_at_end() && self.peek().kind != TokenKind::RBrace {
                    match self.parse_var_decl() {
                        Ok(Stmt::VarDecl { name, type_node, .. }) => {
                            constructor.params.push(crate::parser::ast::Param { name, type_node });
                        }
                        Ok(_) => {
                            return Err("Syntax Error: Expected variable declaration in param block".to_string());
                        }
                        Err(e) => return Err(e),
                    }
                }
                self.consume(TokenKind::RBrace, "Expected '}' to close constructor param block")?;
            ...
            while !self.is_at_end() && self.peek().kind != TokenKind::RBrace {
                constructor.body.push(self.parse_statement()?.expect("REASON"));
            }
    ...
    pub(crate) fn parse_constructor_decl(&mut self) -> Result<ConstructorDecl, String> {
        ...
        if self.peek().kind != TokenKind::RParen {
            loop {
                ...
                params.push(Param { name, type_node: Some(type_node) });
        ...
        self.consume(TokenKind::Arrow, "Expected '->' after constructor signature '_(...)'")?;
